 ...
 * team_members, team,
 * project_members, client_primary_contact, client_all_contacts, task_assignee, task_collaborators, comment_creator, leave_applicant, ticket_creator
 */
if (!function_exists('get_notification_config')) {
    ...
    function get_notification_config($event = "", $key = "", $info_options = array()) {
        ...
        $events = array(
            ...
            "project_team_members_feedback_added" => array(
                "notify_to" => array("client_all_contacts","client_primary_contact", "team_members"),
                "info" => $project_link
            ),
            ...
            "ticket_commented" => array(
                "notify_to" => array("client_primary_contact", "client_all_contacts", "ticket_creator", "team_members", "team"),
                "info" => $ticket_link
            ),
            "ticket_closed" => array(
                "notify_to" => array("client_primary_contact", "client_all_contacts", "ticket_creator", "team_members", "team"),
...
if (!function_exists('send_notification_emails')) {
    ...
    function send_notification_emails($notification_id, $email_notify_to) {
        ...
        if ($notification->category == "ticket") {
            ...
            $notification_event = str_replace ("notification_project_customer_feedback_replied", "notification_project_customer_feedback_added", "notification_" . $notification->event);
            $parser_data["EVENT_TITLE"] = "<b>" . $notification->user_name . "</b> " . sprintf(lang($notification_event), $notification->to_user_name);
        ...
        $parser_data["EVENT_TITLE"] = $notification->user_name . " " . sprintf(lang($notification_event), $notification->to_user_name);
        $subject = $ci->parser->parse_string($email_template->subject, $parser_data, TRUE);
        
        send_app_mail("", $subject, $message, array("bcc" => $email_notify_to));
        /* 
        if( "notification_" . $notification->event == 'notification_project_team_members_feedback_added' ){
            send_app_mail($email_notify_to, $subject, $message, array());
        } else {
            send_app_mail("", $subject, $message, array("bcc" => $email_notify_to));
        }
         */
