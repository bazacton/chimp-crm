...
class Clients_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        LEFT JOIN (SELECT client_id, COUNT(id) AS total_projects FROM $projects_table WHERE deleted=0 AND type='project' GROUP BY client_id) AS project_table ON project_table.client_id= $clients_table.id
        LEFT JOIN (SELECT client_id, SUM(payments_table.payment_received) as payment_received, SUM(items_table.invoice_value + IFNULL(items_table.invoice_value*tax_table.percentage/100,0) + IFNULL(items_table.invoice_value*tax_table2.percentage/100,0)) as invoice_value FROM $invoices_table
                   ...
                   LEFT JOIN (SELECT invoice_id, SUM(amount) AS payment_received FROM $invoice_payments_table WHERE deleted=0 GROUP BY invoice_id) AS payments_table ON payments_table.invoice_id=$invoices_table.id AND $invoices_table.deleted=0
    ...
    function is_company_exists($company_name) {
