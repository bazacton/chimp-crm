...
class Notifications_model extends Crud_model {
    ...
    function create_notification($event, $user_id, $options = array()) {
        ...
        if (in_array("ticket_creator", $notify_to_terms) && $ticket_id) {
            $where.= " OR ($users_table.id=((SELECT $tickets_table.client_id FROM $tickets_table WHERE $tickets_table.id=$ticket_id))) ";
        }


        //find project comment creator, comment id is not = id. It should be the the original comment_id
        if (in_array("comment_creator", $notify_to_terms) && $comment_id) {
            $where.= " OR ($users_table.id=(SELECT $project_comments_table.created_by FROM $project_comments_table WHERE $project_comments_table.id=$comment_id)) ";
        }
        ...
        if ($notify_to->num_rows()) {
            foreach ($notify_to->result() as $user) {
                ...
                if ($user->enable_web_notification) {
                    if ($web_notify_to) {
                        $web_notify_to .=",";
                    }
